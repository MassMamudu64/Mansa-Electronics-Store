...
export function renderOrderConfirmation(o: OrderEmailPayload): {
  subject: string;
  html: string;
} {
  ...
  const rows = o.items
    .map(
      (i) => `
        <tr>
          <td style="padding:10px 12px;border-bottom:1px solid #e5e5ea;font-size:14px;color:#111">${escape(i.name)}</td>
  ...
  const subject = `Your Mansa Electronics order ${o.orderRef} is confirmed`;
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        <tr>
          <td style="background:#111;padding:20px 28px">
            ...
            <p style="margin:4px 0 0;font-size:12px;color:#9b9b9f;letter-spacing:0.1em">ORDER CONFIRMATION</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px">
            <h1 style="margin:0 0 12px;font-size:22px;font-weight:800;color:#111">Thanks, ${escape(o.customerName)}!</h1>
            <p style="margin:0 0 4px;font-size:15px;color:#3a3a3c;line-height:1.5">
              We've received your order and it's being prepared. You'll get another email when it ships.
            </p>
            <p style="margin:16px 0 24px;font-size:13px;color:#636366">
              Order ID: <strong style="color:#111">${escape(o.orderRef)}</strong><br>
              Placed: ${escape(new Date(o.createdAt).toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'short' }))}
            </p>

            <h2 style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#636366">Order summary</h2>
            ...
            <p style="margin:0;font-size:14px;color:#3a3a3c;line-height:1.6">
              Thank you for shopping with Mansa Electronics. If you have any questions about this order, just reply to this email.
            </p>
          </td>
        </tr>
        <tr>
          <td style="background:#f2f2f7;padding:16px 28px;border-top:1px solid #e5e5ea">
            <p style="margin:0;font-size:11px;color:#8e8e93;text-align:center">
              Mansa Electronics · This confirmation was sent to ${escape(o.customerEmail)}
            </p>
          </td>
        </tr>
      </table>
